...
void ui_draw(nk_context* ctx, game_state* game) {
      ...
      if (selectedTabs == SYSTEM) {
        ...
        nk_layout_row_dynamic(ctx, 20, 1);
        {
          ui_label(ctx, NK_TEXT_LEFT, 64, "Speed (km/h) %.3f ",
                   glms_vec3_norm(chassis->velocity) / 1000.f * 60.f * 60.f);
          ui_label(ctx, NK_TEXT_LEFT, 64, "Velocity (m/s) %.3f %.3f %.3f ",
                   chassis->velocity.x, chassis->velocity.y,
                   chassis->velocity.z);
          ui_label(ctx, NK_TEXT_LEFT, 128, "Slip ratio %.3f %.3f %.3f %.3f",
                   game->car.wheelHinge[0].slipRatio,
                   game->car.wheelHinge[1].slipRatio,
                   game->car.wheelHinge[2].slipRatio,
                   game->car.wheelHinge[3].slipRatio);
        }
