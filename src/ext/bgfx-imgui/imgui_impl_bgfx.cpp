...
void ImGui_Implbgfx_RenderDrawLists(ImDrawData* draw_data) {
  ...
  mat4 ortho = GLM_MAT4_ZERO_INIT;
  // bx::mtxOrtho(
  glm_ortho(0.0f, io.DisplaySize.x, io.DisplaySize.y, 0.0f, 1000.0f, 0.0f,
            ortho);
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        const uint16_t xx = (uint16_t)fmax(pcmd->ClipRect.x, 0.0f);
        const uint16_t yy = (uint16_t)fmax(pcmd->ClipRect.y, 0.0f);
        bgfx::setScissor(xx, yy,
                         (uint16_t)fmin(pcmd->ClipRect.z, 65535.0f) - xx,
                         (uint16_t)fmin(pcmd->ClipRect.w, 65535.0f) - yy);
